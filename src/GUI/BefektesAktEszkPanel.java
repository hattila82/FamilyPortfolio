...
/**
 *
 * @author HorvathAttila BefektesAktEszkPanel osztály létrehozása, panel
 * felépítése, eseménykezelés
 */
public class BefektesAktEszkPanel extends JPanel implements ActionListener {
    ...
    private CategoryDataset createDataset() {
        ...
        try {
            if (goodsComboFieldAdd.getSelectedItem().equals("Mind") && szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("2", chooseDate, "%", "%");
            }
        } catch (Exception e) {
        }
        try {
            if (!goodsComboFieldAdd.getSelectedItem().equals("Mind") && szemelyCombo.getSelectedItem().equals("Mind")) {
                ...
                Befektetesek = VagyonDB.getVagyonParam3("2", chooseDate, "%", goodsComboFieldAdd.getSelectedItem().toString());
            }
        } catch (Exception e) {
        }
        try {
            if (goodsComboFieldAdd.getSelectedItem().equals("Mind") && !szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("2", chooseDate, szemelyCombo.getSelectedItem().toString(), "%");
            }
        } catch (Exception e) {
        }
        try {
            if (!goodsComboFieldAdd.getSelectedItem().equals("Mind") && !szemelyCombo.getSelectedItem().equals("Mind")) {
                ...
                Befektetesek = VagyonDB.getVagyonParam3("2", chooseDate, szemelyCombo.getSelectedItem().toString(), goodsComboFieldAdd.getSelectedItem().toString());
            }
        } catch (Exception e) {
        }
        try {
            for (int i = 0; i < Befektetesek.size(); i++) {
                ...
                Kiad = 0;
                AktAtvez = 0;
                TranzakcioBev = VagyonDB.getTranzakcioBalancedatum(3, Befektetesek.get(i).getVID(), chooseDate);
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                    AktAtvez = TranzakcioBev.get(k).getSumma();
                }
                AktAtvezLevon = 0;
                TranzakcioBev = VagyonDB.getTranzakcioBalancedatum2(3, Befektetesek.get(i).getVID(), chooseDate);
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                    AktAtvezLevon = TranzakcioBev.get(k).getSumma();
                }
                TranzakcioBev = VagyonDB.getTranzakcioFutureValue3(7, Befektetesek.get(i).getVID());
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                    Bev = Bev + TranzakcioBev.get(k).getPresentValue(Befektetesek.get(i).getNyitoDatum(), model.getValue().toLocalDate());
                }
                TranzakcioKiad = VagyonDB.getTranzakcioFutureValue3(8, Befektetesek.get(i).getVID());
                ...
                if (currencyComboField.getSelectedItem() == "PÉNZNEM") {
                    FirstPrice = Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad;
                } else {
                    ...
                    FirstPrice = (Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad) * (RatioAmit / RatioAmire);
                }
                Bev = 0;
                Kiad = 0;
                AktAtvez = 0;
                TranzakcioBev = VagyonDB.getTranzakcioBalancedatum(3, Befektetesek.get(i).getVID(), chooseDate2);
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                    AktAtvez = TranzakcioBev.get(k).getSumma();
                }
                AktAtvezLevon = 0;
                TranzakcioBev = VagyonDB.getTranzakcioBalancedatum2(3, Befektetesek.get(i).getVID(), chooseDate2);
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                ...
                TranzakcioBev = VagyonDB.getTranzakcioFutureValue3(7, Befektetesek.get(i).getVID());
                for (int k = 0; k < TranzakcioBev.size(); k++) {
                    Bev = Bev + TranzakcioBev.get(k).getPresentValue(Befektetesek.get(i).getNyitoDatum(), model2.getValue().toLocalDate());
                }
                TranzakcioKiad = VagyonDB.getTranzakcioFutureValue3(8, Befektetesek.get(i).getVID());

                for (int k = 0; k < TranzakcioKiad.size(); k++) {
                    Kiad = Kiad + TranzakcioKiad.get(k).getPresentValue(Befektetesek.get(i).getNyitoDatum(), model2.getValue().toLocalDate());
                }

                if (currencyComboField.getSelectedItem() == "PÉNZNEM") {
                    SecondPrice = Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad;
                } else {
                    float RatioAmire = VagyonDB.getPenznemID((String) currencyComboField.getSelectedItem()).get(0).getHUFRatio();
                    float RatioAmit = VagyonDB.getPenznemID((String) Befektetesek.get(i).getPenznemNev()).get(0).getHUFRatio();
                    SecondPrice = (Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad) * (RatioAmit / RatioAmire);
                }
    ...
    public void aktEszkTableLoad() {
        ...
        for (int i = 0; i < Befektetesek.size(); i++) {
            ...
            if (currencyComboField.getSelectedItem() == "PÉNZNEM") {
                row[2] = Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad;
            } else {
                float RatioAmire = VagyonDB.getPenznemID((String) currencyComboField.getSelectedItem()).get(0).getHUFRatio();
                float RatioAmit = VagyonDB.getPenznemID((String) Befektetesek.get(i).getPenznemNev()).get(0).getHUFRatio();
                row[2] = (Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad) * (RatioAmit / RatioAmire);
            }
            ...
            if (currencyComboField.getSelectedItem() == "PÉNZNEM") {
                row[4] = Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad;
            } else {
                float RatioAmire = VagyonDB.getPenznemID((String) currencyComboField.getSelectedItem()).get(0).getHUFRatio();
                float RatioAmit = VagyonDB.getPenznemID((String) Befektetesek.get(i).getPenznemNev()).get(0).getHUFRatio();
                row[4] = (Befektetesek.get(i).getNyitoEgyenleg() + AktAtvez - AktAtvezLevon + Bev - Kiad) * (RatioAmit / RatioAmire);
            }
            if (currencyComboField.getSelectedItem() == "PÉNZNEM") {
    ...
    public BefektesAktEszkPanel() {
        ...
        AktEszkTable.getColumnModel().getColumn(0).setHeaderValue("Befektetési eszköz");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == szemelyCombo) {
            ...
            if (szemelyCombo.getSelectedItem().equals("Mind")) {
                ...
                loadGoodsCombo("2", 0, goodsComboFieldAdd);
                ...
                loadGoodsCombo("2", VagyonDB.getSzemelyNev((String) szemelyCombo.getSelectedItem()).get(0).getId(), goodsComboFieldAdd);
            }
        }
