...
/**
 *
 * @author HorvathAttila HitelAktEszkPanel osztály létrehozása, panel
 * felépítése, eseménykezelés
 */
public class HitelAktEszkPanel extends JPanel implements ActionListener {
    ...
    private CategoryDataset createDataset() {
        ...
        try {
            if (goodsComboFieldAdd.getSelectedItem().equals("Mind") && szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("3", chooseDate, "%", "%");
            }
        } catch (Exception e) {
        }
        try {
            if (!goodsComboFieldAdd.getSelectedItem().equals("Mind") && szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("3", chooseDate, "%", goodsComboFieldAdd.getSelectedItem().toString());
            }
        } catch (Exception e) {
        }
        try {
            if (goodsComboFieldAdd.getSelectedItem().equals("Mind") && !szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("3", chooseDate, szemelyCombo.getSelectedItem().toString(), "%");
            }
        } catch (Exception e) {
        }
        try {
            if (!goodsComboFieldAdd.getSelectedItem().equals("Mind") && !szemelyCombo.getSelectedItem().equals("Mind")) {
                Befektetesek = VagyonDB.getVagyonParam3("3", chooseDate, szemelyCombo.getSelectedItem().toString(), goodsComboFieldAdd.getSelectedItem().toString());
            }
        } catch (Exception e) {
        }
        try {
            for (int i = 0; i < Befektetesek.size(); i++) {
                Bev = 0;
                TranzakcioBev = VagyonDB.getTranzakcioFutureValue2(9, Befektetesek.get(i).getVID());
    ...
    public HitelAktEszkPanel() {
        ...
        AktEszkTable.getColumnModel().getColumn(0).setHeaderValue("Hitel");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == szemelyCombo) {
            ...
            if (szemelyCombo.getSelectedItem().equals("Mind")) {
                loadGoodsCombo("3", 0, goodsComboFieldAdd);
            } else if (!szemelyCombo.getSelectedItem().equals("Mind")) {
                loadGoodsCombo("3", VagyonDB.getSzemelyNev((String) szemelyCombo.getSelectedItem()).get(0).getId(), goodsComboFieldAdd);
            }
        }
